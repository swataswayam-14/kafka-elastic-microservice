import router , {catalogService} from "../catalog.route";
...
describe("Catalog Routes", () => {
    describe("POST /products", () => {
      ...
      test("should response with validation error with 400", async () => {
        const requestBody = mockRequest();
        const response = await request(app)
          .post("/products")
    ...
    describe("PATCH /products/:id", () => {
      ...
      test("should reponse with an internal error code 500", async () => {
        const requestBody = mockRequest();
        const product = ProductFactory.build();
        jest.spyOn(catalogService, "updateProduct")
    ...
    describe("GET /products?limit=0&offset=0", () => {
      ...
      test("should reponse with an internal error code 500", async() => {
        ...
        jest.spyOn(catalogService, "getProducts").mockImplementationOnce(() => Promise.reject("Unable to get products"));
        const response = await request(app)
              .get(`/products?limit=${randomLimit}offset=0`)
              .set("Accept", "application/json")
        expect(response.status).toBe(500);
    ...
    describe("GET /products/:id", () => {
      test("should return a product by an id", async () => {
        const product = ProductFactory.build();
      ...
      test("should reponse with an internal error code 500", async() => {
        const product = ProductFactory.build();
        jest.spyOn(catalogService, "getProduct").mockImplementationOnce(() => Promise.reject("Unable to get product"));
        const response = await request(app)
              .get(`/products/${product.id}`)
              .set("Accept", "application/json");
        expect(response.status).toBe(500);
    ...
    describe("DELETE /products/:id",() => {
      ...
      test("should reponse with an internal error code 500", async() => {
        const product = ProductFactory.build();
        jest.spyOn(catalogService, "deleteProduct").mockImplementationOnce(() => Promise.reject("Unable to delete product"));
        const response = await request(app)
              .delete(`/products/${product.id}`)
              .set("Accept", "application/json")
        expect(response.status).toBe(500);
